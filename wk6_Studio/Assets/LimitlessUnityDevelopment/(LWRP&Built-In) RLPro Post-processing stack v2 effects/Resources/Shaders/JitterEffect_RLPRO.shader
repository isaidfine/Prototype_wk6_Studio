...
#define PI 3.14159265359
	...
	float ONE_Y = 0.0;
	half start;
	float twitchVInterval;

	float onOff(float a, float b, float c, float t) {
		return step(c, sin(t + a * cos(t * b)));
	}
	float onOff(float a, float b, float c, float t, float d) {
		return step(c, tan((start* d) + a * cos(t * b)));
	}
#if VHS_JITTER_V_ON
	...
	float2 twitchVertical(float freq, float2 uv, float t) {
		...
		float vShift = 0.4 * onOff(freq, 3.0, 0.9, t, twitchVInterval);
	...
	float4 Frag(VaryingsDefault i) : SV_Target
	{
		...
		ONE_Y = 1.0 / _ScreenParams.y;
				...
				#if VHS_TWITCH_V_ON		
		float vd = time_;
					p = twitchVertical(0.5 * twitchVFreq, p, vd);
				#endif	
			...
			float2 pn = p;
				...
				return lerp(col1, half4(col, tex2D(_MainTex, p).a), fade);
