Shader "RetroLookPro/ArtefactsEffect"
{
	...
	float4 Frag(VaryingsDefault i) : SV_Target
	{
		...
		half3 fc = tex2D(_MainTex, uv).rgb;
		half3 fl = tex2D(_LastTex, uv).rgb;
		...
		fbb = (
				tex2D(_FeedbackTex, uv).rgb +
				tex2D(_FeedbackTex, uv + float2(one_x, 0.0)).rgb +
				tex2D(_FeedbackTex, uv - float2(one_x, 0.0)).rgb
			  ) / 3.0;
		fbb *= feedbackFade;
		fbn = bm_screen(fbn, fbb);
		return half4(fbn * feedbackColor, tex2D(_MainTex, uv).a * diff);
	}
	...
	float4 Frag1(VaryingsDefault i) : SV_Target
	{
		...
		half3 fbb = tex2D(_FeedbackTex, uv).rgb;
