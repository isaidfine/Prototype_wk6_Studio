...
public sealed class RLProJitter : PostProcessEffectSettings
{
    ...
    [Range(0f, 25f), Tooltip("Twitch interval on Y axes.")]
    public FloatParameter twitchVerticalInterval = new FloatParameter { value = 1f };
    [Space]
...
public sealed class RLPRO_SRP_JitterRenderer : PostProcessEffectRenderer<RLProJitter>
{
    ...
    public override void Render(PostProcessRenderContext context)
    {
        ...
        if (once != settings.twitchVertical.value)
        {
            start = 1;
            once = settings.twitchVertical.value;
        }
        if (settings.twitchVertical.value) start += Time.smoothDeltaTime;
        sheet.properties.SetFloat("start", start);
        sheet.properties.SetFloat("twitchVFreq", settings.verticalFreq);
        sheet.properties.SetFloat("twitchVInterval", settings.twitchVerticalInterval);
